...
// ---------- Betfair: API-Transport ----------
  ...
  async function bfH2H(comp, log) {
    ...
    bfEachMarket(sbBms, (mk, e) => {
      ...
      (mk.runners || []).forEach(rn => {
        ...
        if (nsm) ns[nsm[1].toLowerCase()] = { lay: ly.price, vol: ly.size };
      });
  ...
  async function dbFlush(log) {
    ...
    localStorage.setItem('vbsb_csarb_queue', JSON.stringify(q));
...
// ---------- H2H-Turnier-Remap (Runden-Wechsel selbst heilen) ----------
  ...
  const tourKey2 = name => {
    const m = H2H_ROUND_RE.exec(String(name).trim());
    if (!m) return null;
    const base = norm(m[1]);
...
if (!hit) continue;
  ...
  unsafeWindow.__why = async (mid, comp, optsJson) => {
      ...
      if (tennisSb || lidSport === 'Tennis') {
        ...
        const hasScoreChannel = ch => ch.some(c => {
          return c && ['s2A', 's2B', 's5A', 's5B', 'sdPlusA', 'sdPlusB',
            ...
            's32A', 's32B'].indexOf(String(c.kind)) >= 0;
        });
    ...
    for (const b of bs) {
      ...
      if (/^h?cs\d\d$/.test(k)) {
        ...
        if (!pinSpecs.length) log('  [pts] ' + b.name + ': kein PIN-To-Score-Special');
        else if (!mitLay) log('  [pts] ' + b.name + ': BF ohne Lay je Spieler');
        else log('  [pts] ' + b.name + ': ' + matched + '/' + entries.length +
          ' Spieler mit PIN-Match' + (mitLay !== entries.length ?
  ...
  async function scanLeague(lid, comp, log, rows, games) {
    ...
    for (const { h, b } of pts) {
      const pinSpecs = h.pts || [];
      if (!pinSpecs.length) {
        log('  PTS ' + b.name + ': kein PIN-To-Score-Special');
        continue;
      }
      for (const [player, bf] of Object.entries(b.pts || {})) {
        ...
        if (bf.lay) {
          ...
          if (!candBL) {
            log('  PTS ' + player + ' ' + b.name + ': kein PIN-Special-Match');
          } else {
...
for (const cp of crossPairs2) {
    ...
    for (const g of hfouGot) {
      ...
      const score = price(g.over, g.b.layO) + price(g.under, g.b.layU);
      const m = tryBLPair('HT-O/U ' + g.b.line + ' ' + g.b.name, score,
        [{ kind: 'HT-O/U ' + g.b.line, back: g.over, lay: g.b.layO },
         { kind: 'HT-O/U ' + g.b.line, back: g.under, lay: g.b.layU }],
        g.b.name, log);
      if (!m.ok) continue;
      log('  HT-O/U ' + g.b.line + ' ' + g.b.name + ': Spec ' + g.sid +
        ' (Over ' + g.over.toFixed(2) + ' / Under ' + g.under.toFixed(2) +
        ') Score ' + m.score.toFixed(3));
if (arbDir(g.over, g.b.layO))
        pushRow(rows, lid, { name: g.b.name, hit: g.h, b: g.b,
    ...
    async function processMatchOdds(pairs, period, kindPrefix, srcLabel, logFn) {
      ...
      for (const { h, b } of pairs) {
        ...
        for (const [kind, d, lay, vol] of sides) {
          ...
          if (!(back > 1.01) || !(lay > 0) || !arbDir(back, lay)) continue;
          pushRow(rows, lid, { name: b.name, hit: h, b,
  ...
  async function scanH2HLeague(lid, comp, log, rows, seenRen, games) {
    ...
    for (const sb of bf.sb || []) {
      ...
      for (const i of [0, 1]) {
        const s = layByTeam[i];
        if (!s || !arbDir(h.back2[i], s.s20.lay)) continue;
        sbHit++;
        pushRow(rows, lid, { name: sb.name, hit: h, b: sb,
          kind: 's2' + (i === 0 ? 'A' : 'B'), back: h.back2[i],
      ...
      for (const i of [0, 1]) {
        ...
        if (crossBackEdge(pinPlus, bfEffQ(opp.s20.back)) <= 0) continue;
        sbHit++;
        pushRow(rows, lid, { name: sb.name, hit: h, b: sb,
      ...
      for (const i of [0, 1]) {
        ...
        if (isValidPrice(pinPlus) && opp && isValidPrice(opp.s30.back) &&
            crossBackEdge(pinPlus, bfEffQ(opp.s30.back)) > 0) {
          sbHit++;
          pushRow(rows, lid, { name: sb.name, hit: h, b: sb,
    ...
    for (const sw of bf.sw || []) {
      ...
      swHit++;
      if (arbDir(wk[0], x.lay))
        pushRow(rows, lid, { name: sw.name, hit: h, b: sw,
          kind: 'sw' + sw.per + 'A', back: wk[0],
          src: 'PIN Satz ' + sw.per + ' ' + h.teams[0] + ' / BF Set ' + sw.per + ' Winner',
          lay: x.lay, vol: x.volL });
      if (arbDir(wk[1], y.lay))
        pushRow(rows, lid, { name: sw.name, hit: h, b: sw,
          kind: 'sw' + sw.per + 'B', back: wk[1],
  ...
  // Kombiniertes Gate fuer Paar-Matches (BTTS, HT O/U): kombinierter Score,
  // Richtung wird je Einzelleit geprueft.
  const tryBLPair = (label, priceSum, legs, name, log) => {
    if (priceSum >= PRICE_MATCH_THRESHOLD) {
      for (const l of legs) trackMissed(l.kind, l.back, l.lay, priceSum, name);
      log('  ' + label + ': kein Preismatch (Score ' + priceSum.toFixed(3) + ')');
      return { ok: false, score: priceSum };
    }
    return { ok: true, score: priceSum };
  };
  ...
  let lastActive = [];   // Discovery: {sid, name, leagues:[{id,name}]} fuer "Manuell mappen"
  ...
  function buildPanel() {
    ...
    const render = () => {
      ...
      localStorage.setItem('vbsb_csarb_snapshot', JSON.stringify(snap));
    };
  ...
  function cmdPoll() {
    ...
    GM_xmlhttpRequest({
      method: 'GET', url: CMD_URL, timeout: 5000,
      onload: r => {
        console.log('[Pruefung] GET /cmd -> status ' + r.status);
        if (!(r.status >= 200 && r.status < 300) || r.status === 204) return;
        let cmd = null;
        try { cmd = JSON.parse(r.responseText); } catch (e) { return; }
        if (!cmd || !cmd.id || !cmd.tool) return;
        console.log('[Pruefung] Auftrag erhalten: id=' + cmd.id + ' tool=' + cmd.tool);
        cmdBusy = true;
        runCmd(cmd).then(() => { cmdBusy = false; }).catch(() => { cmdBusy = false; });
      },
      onerror: e => console.log('[Pruefung] GET /cmd Fehler: ' + e),
      ontimeout: () => console.log('[Pruefung] GET /cmd Timeout')
    });
  }
  setInterval(cmdPoll, 4000);
  cmdPoll();
