...
// ---------- Reine Matching-Logik (in Node testbar) ----------
  ...
  const SIDE_ALIAS = {
    ...
    "atletico cp": "atletico clube de portugal",
...
// ---------- Konstanten, Tuning, API-Session-Zustand ----------
  ...
  const reqStats = {
    ...
    // Cache-Zaehler (v8.73.0): Wie oft hat ein Cache einen echten API-Request
    // erspart (Treffer) bzw. wie oft wurde neu geladen (Fehlschlag)? Getrennt
    // nach PIN/BF — die BF-Cache-Quote ist die Messlatte fuer die spaetere
    // Alters-/Diff-Optimierung (bfLaysCache + bfH2HCache, TTL 60 s).
    pinCacheHit: 0, pinCacheMiss: 0, bfCacheHit: 0, bfCacheMiss: 0,
  };
  const reqStatsReset = () => {
    ...
    reqStats.pinCacheHit = 0; reqStats.pinCacheMiss = 0;
    reqStats.bfCacheHit = 0; reqStats.bfCacheMiss = 0;
  };
  ...
  // Clear nur bei Verify — NICHT bei normalem Scan-Start und seit v8.78.2
  // auch nicht mehr beim WHY-Pull (dort entscheidet der 60-s-TTL).
  const bfLaysCache = new Map();
  // H2H-Cross-Scan-Cache je Liga (2-Wege-Maerkte): gleiche Fetches ueber
  // Scans hinweg gueltig (TTL 60s). Clear nur bei Verify (WHY seit v8.78.2
  // ebenfalls TTL-bewertet).
...
// ---------- Betfair: API-Transport ----------
  ...
  async function bfH2H(comp, log) {
    ...
    if (cached && (Date.now() - cached.ts) < BF_CACHE_TTL_MS) {
      reqStats.bfCacheHit++;
      if (DBG) log('  BF-H2H ' + comp + ': cached (' + cached.mo.length + ' Maerkte)');
      return cached;
    }
    reqStats.bfCacheMiss++;
    const j = await bfLeagueMarkets(comp);
...
async function autoTourLid(lid, comp, log) {
    ...
    if (sid === null) {
      ...
      // durchprobieren, bis der lid in der Ligen-Liste auftaucht — ODER bis
      // eine Liga desselben Turniers (gleiche tourKey2-Basis) dort liegt.
      // Letzteres ist der Runden-Wechsel-Fall (v8.78.6): der alte Runden-lid
      // (z.B. "ATP US Open - R16") ist aus der aktuellen PIN-Ligen-Liste
      // bereits entfernt (Runde vorbei), die Folge-Runde ("ATP US Open - QF")
      // steht aber unter demselben Basisnamen in derselben Sportart — ohne
      // diesen Check bleibt sid null und der Remap bricht ab (Befund
      // 08.09.2026: US Open R16→QF, kein Tennis mehr in pvb_odds.db).
      for (const trySid of Object.keys(BF_SID).map(Number)) {
        ...
        if ((ll || []).some(x => {
          const ct = tourKey2(x.name);
          return ct && ct.base === tk.base;
        })) { sid = trySid; break; }
      }
    }
    if (sid === null) {
  ...
  async function h2hRoundSiblings(lid, log) {
    ...
    if (sid === null) {
      // Gleicher Basisnamen-Fallback wie in autoTourLid (v8.78.6): der alte
      // Runden-lid kann aus der aktuellen PIN-Ligen-Liste verschwunden sein
      // (Runde vorbei), die Folge-Runde liegt aber unter derselben
      // tourKey2-Basis in der Sportart — ohne diesen Check bleibt sid null
      // und der Sibling-Merge findet nie statt.
      for (const trySid of Object.keys(BF_SID).map(Number)) {
        if (trySid === 29) continue;
        const ll = await pinGet('/sports/' + trySid + '/leagues').catch(() => []);
        if ((ll || []).some(x => String(x.id) === String(lid))) { sid = trySid; break; }
...
// __htcsprobe(1633236740, 1843) -> zeigt alle Specials + straight-Maerkte eines
  ...
  unsafeWindow.__bfsnapshot = async (sport) => {
    ...
    const SIDS = { soccer:1, tennis:2, basketball:7522, baseball:3, handball:18,
      ...
      mma:22, table_tennis:32, darts:3503, snooker:6422 };
  ...
  const BF_EV_TYPE_CANDIDATES = [1, 2, 33, 29, 3, 4, 5, 1477, 6, 18, 22, 26, 27, 32, 34, 39, 3503, 6422];
...
if (!hit) continue;
  ...
  unsafeWindow.__why = async (mid, comp, optsJson) => {
    ...
    // Frische-Politik (v8.78.2, User-Freigabe „Genauigkeit des Userscripts
    // reicht aus“): Nur der PIN-straightCache wird geleert — er hat KEINEN
    // TTL und darf nicht beliebig alt sein (und ist billig nachzuladen).
    // Die BF-Caches (bfLaysCache/bfH2HCache) bleiben dagegen stehen: ihr
    // 60-s-TTL (BF_CACHE_TTL_MS) entscheidet selbst — Eintraege ≤ 60 s
    // werden wie beim normalen Scan wiederverwendet (Near-Fresh), aeltere
    // werden frisch von Betfair nachgeladen. Das spart den teuren
    // BF-Komplett-COMP-Fetch, wenn kurz vorher ein Scan/why lief, ohne die
    // Datenqualitaet zu verschlechtern (identisch zum Scan-Stand).
    straightCache.clear();
    log('WHY: PIN-straight frisch geleert; BF-Caches TTL-bewertet (≤ 60 s).');
    ...
    if (isH2H) {
      ...
      if (!moH.length) {
        ...
        // v8.79.5: Auch OHNE BF-Event die PIN-ML-Legs als Kanäle liefern —
        // der Boost-Arb kann das Komplement trotzdem bei PIN backen (V2 =
        // Back-Komplement @ PIN), auch wenn kein BF-Lay existiert. Live-Fall
        // 2026-09-09 FIBA World Cup Women: Betfair führt "Belgium v Germany"
        // nicht (COMP:12522790 hat nur 2 der 4 PIN-Spiele) — ohne diese Legs
        // blieb der Back-Back-Kanal über PIN unentdeckt (vorher return out
        // ohne einen einzigen Kanal).
        if (h2 && h2.back && h2.back.length >= 2 && h2.teams && h2.teams.length >= 2) {
          const a = h2.back[0], bb = h2.back[1];
          add('bl A', a > 1.01 ? a : null, null, 'PIN A (' + h2.teams[0] + ')',
            'BF kein Event', 'V2: Back-Komplement @ PIN moeglich');
          add('bl B', bb > 1.01 ? bb : null, null, 'PIN B (' + h2.teams[1] + ')',
            'BF kein Event', 'V2: Back-Komplement @ PIN moeglich');
        }
        return out;
      }
      ...
      for (const b of moH) {
        ...
        const a = h2.back[0], bb = h2.back[1];
        // xBack (v8.79.8): BF-Back des Gegner-Runners als 8. add-Parameter —
        // das Back-Back-Crossleg (V3 „Back ¬M @ Betfair") ist im DB-Pfad
        // (scan.js blA/blB) vorhanden, fehlte aber im why-Pull: bf_komp_back
        // blieb 0 und der Solver uebersprang V3. Analog scan.js (isValidPrice
  ...
  const TOOLS = [    { id: 'bfht', group: 'diagnose', label: 'BF Marktnamen/FIlter', desc: 'Listet Marktnamen sowie optional gefilterte Maerkte einer COMP.',
    ...
    // Eigenstaendiges bfnav-Tool (v8.72.0): Navigationsbaum einer BF-
...
for (const cp of crossPairs2) {
    ...
    async function processMatchOdds(pairs, period, kindPrefix, srcLabel, logFn) {
      ...
      for (const { h, b } of pairs) {
        ...
        let sHome = sideLay(h.teams[0], b);
        let sAway = sideLay(h.teams[1], b);
        // v8.79.11: Doppel-lid-Seitenkorrektur — statt die Zeile bei absurder
        // Marge nur zu verwerfen (v8.78.7), erst pruefen, ob die Runner-
        // Zuordnung vertauscht ist (PIN-Spiel ueber zwei lids -> BF-Lay vom
        // falschen Runner, Rakow-Fall 2026-09-08). Die pure Funktion liefert
        // die korrigierte Zuordnung, wenn die vertauschte auf beiden Seiten
        // plausibel ist; nur echte Fremd-Lays bleiben beim Verwerf-Guard.
        const backHome = toDec(px['home'] ?? px['tie']);
        const backAway = toDec(px['away'] ?? px['tie']);
        const korr = korrigiereSeitenZuordnung(backHome, backAway, sHome, sAway);
        if (korr.korrigiert) {
          log('  DEBUG Doppel-lid korrigiert[' + lid + '] ' + b.name + ': ' +
            'Seiten vertauscht (home ' + (sHome && sHome.lay) + ' -> ' +
            (korr.sHome && korr.sHome.lay) + ', away ' + (sAway && sAway.lay) +
            ' -> ' + (korr.sAway && korr.sAway.lay) + ')');
        }
        sHome = korr.sHome;
        sAway = korr.sAway;
        const sides = [
    ...
    for (const { h, b } of hfs) {
      for (const o of h.hfs || []) for (const out of o.outs || []) {
        ...
        // v8.78.1: HT/FT-Rows auch OHNE Arb-Richtung speichern (analog ML/h3
        // v8.62.12, CS v8.60.34): Der Boost-Arb-Solver braucht PIN-HT/FT-Back
        // + BF-HT/FT-Lay fuer den DB-Schnellpfad auch dann, wenn PIN-Back >
        // BF-Lay (Normalfall Marge). Vorher verwarf arbDir() fast alle Rows →
        // der Snapshot hatte nie hfHH/hfHD/… und der Boost-Check fiel bei
        // HT/FT-Kombis immer in den langsamen why-Pull (Browser-Komplett-
  ...
  async function scanH2HLeague(lid, comp, log, rows, seenRen, games) {
    ...
    if (!bf.mo.length && !(bf.sb || []).length && !(bf.sw || []).length && !(bf.ou || []).length && !(bf.oe || []).length && !(bf.gd || []).length) {
      ...
      // v8.79.11 (Doppel-lid-Seitenkorrektur, h2h-Zweig): Analog zum CS-Pfad
      // (processMatchOdds) kann der Scanner bei Doppel-lid-Verarbeitung dem
      // PIN-Spiel den BF-Lay vom falschen Runner zuordnen (Rakow-Fall war
      // Soccer h3; Tennis/MMA/Boxen blA/blB sind derselbe Mechanismus). Die
      // pure Funktion prueft, ob die vertauschte Zuordnung auf beiden Seiten
      // plausibel ist, und liefert die korrigierten Runner zurueck — statt
      // die Zeile mit falschem Lay in die DB zu schreiben (die App wuerde sie
      // nur spaeter wegtolerieren, die korrekten Schwester-Zeilen fehlten).
      const korr = korrigiereSeitenZuordnung(a, bb, x, y);
      if (korr.korrigiert) {
        log('  DEBUG Doppel-lid korrigiert[' + lid + '] ' + b.name + ': ' +
          'Runner vertauscht (A ' + (x && x.lay) + ' -> ' +
          (korr.sHome && korr.sHome.lay) + ', B ' + (y && y.lay) +
          ' -> ' + (korr.sAway && korr.sAway.lay) + ')');
      }
      x = korr.sHome;
      y = korr.sAway;
  ...
  // ---------- Doppel-lid-Seitenkorrektur (pure, Node-testbar) ----------
  // v8.79.11: Bei Doppel-lid-Verarbeitung kann der Browser-Scanner einem
  // PIN-Spiel den BF-Lay vom FALSCHEN Runner zuordnen (Rakow-Fall 2026-09-08:
  // h3A back=1.72 mit lay=11.5 statt ~2.48). v8.78.7 verwarf solche Zeilen
  // an der Quelle (Marge > 2.5x) — die korrekten Schwester-Zeilen fehlten
  // damit weiterhin. Diese Funktion erkennt die vertauschte Zuordnung und
  // liefert die korrigierte Seiten-Zuordnung zurueck, statt zu verwerfen:
  // Wenn die urspruengliche Zuordnung absurd ist (Marge > maxMarge), die
  // vertauschte aber auf BEIDEN Seiten plausibel waere, ist die Zuordnung
  // vertauscht. Reine Funktion (kein Browser-Zugriff) — aufgerufen aus
  // processMatchOdds (scan.js), getestet in test/snapshot.test.js.
  function korrigiereSeitenZuordnung(backHome, backAway, sHome, sAway, maxMarge) {
    const lim = maxMarge || 2.5;
    if (!sHome || !sAway || !(backHome > 1.01) || !(backAway > 1.01)) {
      return { sHome, sAway, korrigiert: false };
    }
    const mH = sHome.lay / backHome;
    const mA = sAway.lay / backAway;
    const mSwapH = sAway.lay / backHome;
    const mSwapA = sHome.lay / backAway;
    // Vertauscht genau dann, wenn die urspruengliche Zuordnung absurd ist,
    // die vertauschte aber auf beiden Seiten unter der Schwelle bleibt.
    if ((mH > lim || mA > lim) && mSwapH <= lim && mSwapA <= lim) {
      return { sHome: sAway, sAway: sHome, korrigiert: true };
    }
    return { sHome, sAway, korrigiert: false };
  }
  ...
  function sportVonLiga(lid) {
    ...
    if (/darts|pdc|modus super series|world series of darts/.test(n)) return 'Darts';
    if (/snooker|uk championship|english open|welsh open|scottish open|northern ireland open|german masters|players championship|champion of champions|tour championship|world grand prix|british open|international championship|shanghai masters|world snooker|home nations|shoot out|wuhan open|saudi masters|european masters|xi'an grand prix|championship league|six-red|world mixed doubles|world open|invitational/.test(n)) return 'Snooker';
    // BF-EventType-basierter Sport-Fallback (v8.79.11, generalisiert aus dem
    // v8.75.1-Snooker-Sonderfall): Fuer neue/zukuenftige Turniere, deren Name
    // keinen Sport-Marker traegt (z.B. FIBA-Weltcups ohne "fiba" im Namen,
    // "International Friendlies"), liefert die gemappte COMP den eindeutigen
    // BF-EventType (effSid) — sobald sie einmal via nodeInfo geladen wurde
    // (Discovery-Lauf, manuelles Mapping). Der Cache lebt in discovery.js
    // (kommt im Build vor main.js, gleicher IIFE-Scope). Die Tabelle ist die
    // Invertierte von BF_SID (PIN-Sport-ID -> BF-EventType-ID) und muss mit
    // ihr konsistent bleiben (siehe BF_SID-Kommentar bei bfNode).
    if (H2H[lid]) {
      const cid = Number(String(H2H[lid]).replace(/^COMP:/, ''));
      const info = cid && bfNodeCache.get(cid);
      const sport = info && BF_ET_SPORT[info.effSid];
      if (sport) return sport;
    }
    // H2H-Ligen ohne erkennbaren Sport-Marker UND ohne bekannte COMP
  ...
  // BF-EventType-ID -> Sportname (Inverse der BF_SID-Karte unten).
  // 6 (Boxing) -> MMA, da sportVonLiga Boxing unter MMA fuehrt; 5/1477
  // (Rugby Union/League) -> Rugby.
  const BF_ET_SPORT = {
    1: 'Soccer', 2: 'Tennis', 7511: 'Baseball', 7522: 'Basketball',
    4: 'Cricket', 468328: 'Handball', 998917: 'Volleyball',
    7524: 'Ice Hockey', 6423: 'American Football', 61420: 'Aussie Rules',
    26420387: 'MMA', 6: 'MMA', 27454571: 'Esports', 5: 'Rugby', 1477: 'Rugby',
    3503: 'Darts', 6422: 'Snooker',
  };
  ...
  function buildPanel() {
    ...
    const scan = async (auto) => {
      ...
      // Nur bei Verify aktiv leeren (WHY seit v8.78.2 ebenfalls TTL-bewertet).
      ...
      const cacheHits = reqStats.pinCacheHit + reqStats.bfCacheHit;
      const cacheMisses = reqStats.pinCacheMiss + reqStats.bfCacheMiss;
      log('Fertig. ' + total + ' Ligen, ' + scanSec.toFixed(0) + 's | '
        + matched + ' gematcht → ' + arbs.length + ' Arbs | '
        + 'PIN ' + reqStats.pin + ' / BF ' + reqStats.bf + ' Req | '
        + 'Cache ' + cacheHits + ' Treffer / ' + cacheMisses + ' Misses'
        + (totalErrs ? ' | ' + totalErrs + ' Fehler (' + reqStats.pin429 + '/'
          + reqStats.bf429 + 'x 429)' : ''),
        'color:#9aa3b2;');
      ...
      try {
        ...
        postPipe('/scan-metrics', {
          ...
          cache_hits_pin: reqStats.pinCacheHit, cache_misses_pin: reqStats.pinCacheMiss,
          cache_hits_bf: reqStats.bfCacheHit, cache_misses_bf: reqStats.bfCacheMiss
        });
